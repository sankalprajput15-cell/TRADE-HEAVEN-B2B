...
type ModalTab = 'BASIC' | 'PRICING' | 'QUOTAS' | 'MODELS' | 'FEATURES';
...
export const PlanEditModal: React.FC<PlanEditModalProps> = ({
  ...
  // Pricing
  const [monthlyPriceUsd, setMonthlyPriceUsd] = useState(29);
  const [annualPriceUsd, setAnnualPriceUsd] = useState(290);
  const [discountPercentage, setDiscountPercentage] = useState(17);
  const [currency, setCurrency] = useState<Currency>('USD');

  // Token & Rate Limits
  const [tokenQuotaMonthly, setTokenQuotaMonthly] = useState(35000000);
  ...
  const [allowedModels, setAllowedModels] = useState<GeminiModelId[]>(['gemini-2.5-flash', 'gemini-1.5-pro']);
  ...
  useEffect(() => {
    if (plan) {
      ...
      setTokenQuotaMonthly(plan.tokenQuotaMonthly);
      setRpm(plan.rpm);
      setRpd(plan.rpd);
      setTpm(plan.tpm || 500000);
      setMaxContextWindow(plan.maxContextWindow);
      setMaxOutputTokens(plan.maxOutputTokens);
      setMaxConcurrentRequests(plan.maxConcurrentRequests || 8);

      setAllowedModels(plan.allowedModels || []);
      setFeatureKeys(plan.featureKeys || []);
    } else {
      setName('');
      setSlug('');
      setDescription('');
      setTierBadge('STARTER');
      setStatus('ACTIVE');
      setIsPopular(false);
      setDisplayOrder(1);

      setMonthlyPriceUsd(29);
      setAnnualPriceUsd(290);
      setDiscountPercentage(17);
      setCurrency('USD');

      setTokenQuotaMonthly(25000000);
      setRpm(60);
      setRpd(10000);
      setTpm(500000);
      ...
      setAllowedModels(['gemini-2.5-flash', 'gemini-1.5-pro', 'text-embedding-004']);
  ...
  const toggleModel = (modelId: GeminiModelId) => {
    if (allowedModels.includes(modelId)) {
      if (allowedModels.length === 1) {
        setErrors(prev => ({ ...prev, models: 'At least one Gemini model must be enabled for this plan.' }));
        return;
      }
  ...
  const validate = (): boolean => {
    ...
    if (allowedModels.length === 0) newErrors.models = 'Select at least one allowed Gemini model.';
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-white flex items-center gap-2">
                {isEditing ? `Edit Plan: ${plan?.name}` : 'Create New Gemini SaaS Plan'}
              ...
              <p className="text-xs text-slate-400">
                Configure base pricing, Gemini model rate limits, token quotas, and Stripe synchronization.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 border-b border-slate-200 bg-slate-50 flex items-center gap-1 overflow-x-auto shrink-0">
          ...
          >
            ...
            <span>4. Model Access ({allowedModels.length})</span>
          </button>
          ...
          >
            ...
            <span>5. Feature Flags ({featureKeys.length})</span>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === 'BASIC' && (
            <div className="space-y-5 animate-in fade-in duration-150">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Plan Description &amp; Target Persona
                </label>
                <textarea
                  rows={3}
                  value={description}
                  onChange={e => setDescription(e.target.value)}
                  placeholder="e.g., Flagship tier for production agents with 2M token context window, reasoning, and high RPM."
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      Hard or soft ceiling on tokens processed per billing cycle across Gemini models.
                    </p>
                  </div>
          ...
          {/* TAB 4: ALLOWED GEMINI MODELS */}
          {activeTab === 'MODELS' && (
            <div className="space-y-4 animate-in fade-in duration-150">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xs font-bold text-slate-900">Assigned Google AI Studio Models</h3>
                  <p className="text-[11px] text-slate-500">Check which Gemini models subscribers of this plan have authorization to call.</p>
                </div>
          ...
          <div className="flex items-center gap-2">
            {activeTab !== 'BASIC' && (
              <button
                type="button"
                onClick={() => {
                  const tabs: ModalTab[] = ['BASIC', 'PRICING', 'QUOTAS', 'MODELS', 'FEATURES'];
